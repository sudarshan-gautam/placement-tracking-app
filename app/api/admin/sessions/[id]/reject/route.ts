...
export async function POST(
  ...
  try {
    ...
    // Update session approval status and store rejection reason
    await db.run(
      'UPDATE sessions SET approval_status = ?, rejection_reason = ?, updated_at = datetime("now") WHERE id = ?',
      ['rejected', reason || 'No reason provided', sessionId]
    );
    ...
    return NextResponse.json({ error: 'Failed to reject session' }, { status: 500 });
  }
} 